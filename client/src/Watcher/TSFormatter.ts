...
class TSFormatter {
    ...
    public static Format(input: string, path: string): ITSFile {
        ...
        for(const element of this.CommonJS){
            if (path.indexOf(sep + element + sep) > -1) {
                return this.FormatCommonJS(input, path);
            }
        }
        
        const response: ITSFile = {methods: [], namespace: "", path: path, commonJS: false};
        let namespace: RegExpMatchArray;
        ...
        try {
            ...
            if (input.indexOf("namespace") > -1) {
                const shorten = input.substring(input.indexOf("namespace") - 1, input.indexOf("namespace") + 200);
                namespace = shorten.match(/(namespace\s?)(.*[^\s])(\s?{)/);
            } else if (input.indexOf("module") > -1) {
                const shorten = input.substring(input.indexOf("module") - 1, input.indexOf("module") + 200);
     ...
     */
    private static GetMethods(input: string, commonJS = false): string[] {
